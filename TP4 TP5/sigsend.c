...
int main(int argc, char* argv[]) {
    ...
    int status /* = TODO */ = kill(pid,signum) ;
    if (status == -1) {
...
/* QUESTION 4 : [] veut dire que les arguments sont facultatifs et <> obligatoires, il faut supprimer cela
rentrer l'id qui est présent dans le sigcatch, du genre 8000 et quelques (un grand nombre) */

/* QUESTION 5 : non, CTRL-C envoie des SIGINT qui ne sont pas rattrapables, cela devrait juste envoie
SIGINT avec un numéro à l'écran */

/* QUESTION 6 : non */

/* QUESTION 7 : le programme a reçu le signal STOP, donc il ne reçoit par le SIGSUR1, ni le CTRL-C */
/* Le SIGCONT réactive le programme, et exécute les commandes en attente SIGSUR1 et CTRL-C */

/* QUESTION 8 : morale : ne sert à rien de spammer, le SIGCONT envoie uen fois le signal, 
SIGKILL et CTRL-C pour arrêter le programme, le CTRL-Z permet de le mettre en pause et SG le réveiller */